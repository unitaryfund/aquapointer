    ...
    for i1 in range(len(x)):
        ...
        for k in range(1, p + 1):
            ...
            res += (
                binom(p, k)
                * (-1) ** k
                * integral(p, k, indices, partition, base, component, params)
            )
        # second order
        for i2 in range(len(x)):
            ...
            for j in range(1, p // 2 + 1):
                ...
                res += (
                    binom_even
                    * fact_even
                    / (2 * np.math.factorial(j) ** 2)
                    * integral(p, 2 * j, indices, partition, base, component, params)
                )
                for l in range(1, j):
                    partition = [j - l, j + l]
                    res += (
                        binom_even
                        * fact_even
                        / (np.math.factorial(j - l) * np.math.factorial(j + l))
                        * integral(
                            p, 2 * j, indices, partition, base, component, params
                        )
                    )
            # k = 2j+1 odd
            for j in range(1, (p - 1) // 2 + 1):
                ...
                for l in range(j):
                    ...
                    res -= (
                        binom_odd
                        * fact_odd
                        / (np.math.factorial(j - l) * np.math.factorial(j + l + 1))
                        * integral(
                            p, 2 * j + 1, indices, partition, base, component, params
                        )
                    )
    ...
    for i1 in range(len(x)):
        ...
        for k in range(1, p + 1):
            res += (
                binom(p, k)
                * (-1) ** k
                * integral(p, k, [i1], [k], base, component, params)
            )
