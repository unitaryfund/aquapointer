...
class Lattice:
    ...
    def __init__(self, coords: ArrayLike, **kwargs):
    ...
    @classmethod
    def poisson_disk(
        cls, density: ArrayLike, length: tuple, spacing: tuple, max_num: int = 8000
    ):
        """
        ...
        first_point = np.array(
            [np.random.rand() * length_x, np.random.rand() * length_y]
        )
    ...
    def dynamics(
        ...
        def _leapfrog():
            ...
            while t < T:
                ...
                for i in range(len(vs)):
                    vs[i] += (
                        (_LennardJones_force(i) - _gradient(i) - _viscosity(i)) * dt / 2
                    )
        ...
        def _gradient(i):
            ...
            field_contribution = np.array(
                [grad[1][idx_x, idx_y], grad[0][idx_x, idx_y]]
            )
        ...
        A = (
            1e-4 * 4 * np.max(density)
        )  # the depth of the lennard-jones potential is A/4
