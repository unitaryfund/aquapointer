...
def plot_density(density, rescaled_positions=None, figsize=(10,6), vmin=None, vmax=None, title=None, name=None):
...
def plot_energy_landscape_contour(alphas, betas, energy_levels, levels=8, figsize=(10,6)):
    """
    Plots the energy level of the true solution in the Rydberg Hamiltonian
    as a function of:
    alpha = scale factor between the Rydberg detuning and the linear term in the problem Hamiltonian
...
def ising_energies(nodes, density, var, bitstrings, brad, amp=1):
    ...
    for i, bitstring in enumerate(bitstrings):
        if not bmu.is_IS(bitstring, nodes, brad):
            continue
        mixture = gaussian_mixture(density.shape, var, positions_from_bitstring(bitstring, nodes))
        energy = 0
        for k1 in range(n):
            if int(bitstring[k1]):
...
def compute_cubic_up(rescaled_positions, brad, gamma_list, V_list, U_list, distances, alpha=1, beta=0, C6=5420158.53):
    ising = 0
    rydberg = 0
...
def compute_cubic_down(rescaled_positions, brad, gamma_list, V_list, U_list, distances, alpha=1, beta=0, C6=5420158.53):
    ising = 0
    rydberg = 0
    n = len(rescaled_positions)
    for k1 in range(n):
